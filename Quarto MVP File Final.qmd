...
```{r tbl-points-assists}
#| label: tbl-points-assists
#| tbl-cap: "Points and Assists Comparison for MVP Candidates"

# Primary Author: Naman Joshi
# Reviewer: Kevin Nguyen
# This table compares the MVP candidates by points and assists per game.
...
knitr::kable(namanpointsassiststable)
```

This tabular format further shows the positive relationship between points per game and assists per game, as seen in the scatterplot.
...
pointsPositionPlot
```
```{r tbl-position-scoring}
#| label: tbl-position-scoring
#| tbl-cap: "Scoring Summary by NBA Position"
...
knitr::kable(positionscoringtable)
```

This figure @fig-boxplot and the table shows the distribution of ppg across positions. The boxplots display the median, quartiles, and potential outliers for each position, and this provides a insight on how scoring is different per position.
